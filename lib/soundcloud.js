...
module.exports = {
	...
	getIdUserRecsList: function(userFavoritesUrl) {
		var d = q.defer(),
			relatedReqPromises = [];

		getLikedTracksIds(userFavoritesUrl).then(function(trackIDs) {
...
function getTracksIds(playlistUrl) {
	...
	https.get("https://api.soundcloud.com/resolve.json?client_id=" + clientID + "&url=" + encodeURIComponent(playlistUrl), function(res) {
		...
		res.on('end', () => {
			var tracks = JSON.parse(body.toString()).tracks;
...
function getLikedTracksIds(playlistUrl) {
