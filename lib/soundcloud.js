	...
	getRecommendations: function (soundCloudURL) {
		...
		gettrackIDs(soundCloudURL).then(function (trackIDs) {
			...
			q.all(relatedTrackListPromises).then((relatedTrackLists) => {
				...
				//Remove any related tracks that were in the original list of tracks to recommend from
				for (var i = 0; i < relatedTracks.length; i++) {
					if (trackIDs.indexOf(relatedTracks[i].id) > -1) {
						relatedTracks.splice(i, 1);
						i--;
					}
				}
...
function getRelatedTracksForTrackID(trackID) {
	...
	https.get("https://api-v2.soundcloud.com/tracks/" + trackID + "/related?client_id=" + clientID, function (res) {
		var body = '';
